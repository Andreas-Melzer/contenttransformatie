import streamlit as st
import threading
from typing import Literal, Dict, Optional

from config.settings import settings
from interface.project import Project
from interface.utils.callbacks import (list_documents_callback,
                                       streamlit_tool_callback,
                                       search_results_callback)
from llm_client.agent import MultiTurnAgent
from llm_client.document_vector_store import DocumentStore, VectorStore
from llm_client.llm_client import EmbeddingProcessor, LLMProcessor
from llm_client.prompt_builder import PromptBuilder
from interface.implementations.tools.document_relevance_tool import DocumentRelevanceTool
from interface.implementations.tools.list_selected_documents_tool import ListSelectedDocumentsTool
from interface.implementations.tools.read_documents_tool import ReadDocumentsTool
from interface.implementations.tools.vector_search_tool import VectorSearchTool
from interface.implementations.tools.save_consolidated_json_tool import SaveConsolidatedJsonTool
from interface.implementations.tools.save_rewritten_json_tool import SaveRewrittenJsonTool
from interface.utils.timer import Timer

AgentType = Literal["search", "consolidate", "rewrite"]

class GlobalAppStore:
    """
    A thread-safe singleton class that holds all heavy components
    (LLM, stores) and manages the agent factory.
    
    This object is initialized only ONCE.
    """
    _instance: Optional['GlobalAppStore'] = None
    _lock: threading.Lock = threading.Lock()

    def __new__(cls):
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    cls._instance = super(GlobalAppStore, cls).__new__(cls)
                    cls._instance._initialized = False
        return cls._instance

    def __init__(self):

        if self._initialized:
            return
            
        with self._lock:
            if self._initialized:
                return
                
            print("--- GLOBAL SINGLETON CLASS: Initializing... ---")
            
            self.llm: Optional[LLMProcessor] = None
            self.doc_store: Optional[DocumentStore] = None
            self.vector_store: Optional[VectorStore] = None
            
            self.agents: Dict[str, MultiTurnAgent] = {}
            
            self._load_heavy_components_internal()
            
            self.AGENT_INITIALIZERS = {
                "search": self._initialize_search_agent_for_project,
                "consolidate": self._initialize_consolidate_agent_for_project,
                "rewrite": self._initialize_rewrite_agent_for_project,
            }
            
            self._initialized = True
            print("--- GLOBAL SINGLETON CLASS: Initialization complete. ---")

    def _load_heavy_components_internal(self):
        """
        Internal function to load heavy components.
        This runs only ONCE.
        """
        print("--- GLOBAL SINGLETON: Initializing heavy components (LLM, Embedder, Stores)... ---")
        llm_client_name = settings.llm_client_map.get(settings.llm_model)
        if not llm_client_name or llm_client_name not in settings.clients:
            raise ValueError(f"Client '{llm_client_name}' for model '{settings.llm_model}' not found or configured in settings.")

        llm_config_dict = settings.clients[llm_client_name].copy()
        llm_config_dict['type'] = 'azure' if 'azure' in llm_client_name else llm_client_name

        self.llm = LLMProcessor(
            model=settings.llm_model,
            client_config=llm_config_dict
        )

        embedding_client_name = settings.embedding_client_map.get(settings.embedding_model)
        if not embedding_client_name or embedding_client_name not in settings.clients:
            raise ValueError(f"Client '{embedding_client_name}' for model '{settings.embedding_model}' not found or configured in settings.")
            
        embedding_config_dict = settings.clients[embedding_client_name].copy()
        embedding_config_dict['type'] = 'azure' if 'azure' in embedding_client_name else embedding_client_name

        embedder = EmbeddingProcessor(
            embedding_model=settings.embedding_model,
            client_config=embedding_config_dict
        )
        self.doc_store = DocumentStore(
            settings.raw_doc_store_name,
            settings.docstore_folder,
            settings.indexed_metadata_keys
        )
        self.vector_store = VectorStore(embedder=embedder,
                                        doc_store=self.doc_store,
                                        data_root=settings.docstore_folder)
        
        print("--- GLOBAL SINGLETON: Load complete. ---")

    def _initialize_search_tools(self, project: Project, vector_store: VectorStore, doc_store: DocumentStore):
        on_call_with_project = lambda tool_call: streamlit_tool_callback(tool_call, project)
        on_list_documents = lambda tool_result: list_documents_callback(tool_result, project)

        vector_search_tool = VectorSearchTool(
            vector_store=vector_store,
            on_result=lambda tool_result: search_results_callback(tool_result, project)
        )
        document_relevance_tool = DocumentRelevanceTool(
            on_call=on_call_with_project
        )
        list_tool = ListSelectedDocumentsTool(
            on_result=on_list_documents
        )
        read_tool = ReadDocumentsTool(
            doc_store=doc_store
        )
        return [vector_search_tool, document_relevance_tool, list_tool, read_tool]

    def _initialize_consolidate_tools(self, project: Project, vector_store: VectorStore, doc_store: DocumentStore):
        on_call_with_project = lambda tool_call: streamlit_tool_callback(tool_call, project)
        
        save_consolidated_json_tool = SaveConsolidatedJsonTool(
            project=project,
            on_call=on_call_with_project
        )
        return [save_consolidated_json_tool]

    def _initialize_rewrite_tools(self, project: Project, vector_store: VectorStore, doc_store: DocumentStore):
        on_call_with_project = lambda tool_call: streamlit_tool_callback(tool_call, project)
        
        save_rewritten_json_tool = SaveRewrittenJsonTool(
            project=project,
            on_call=on_call_with_project
        )
        return [save_rewritten_json_tool]

    def _initialize_search_agent_for_project(self, project: Project, llm: LLMProcessor, vector_store: VectorStore, doc_store: DocumentStore) -> MultiTurnAgent:
        """Initialiseert en configureert de search agent voor een specifiek project."""
        agent = MultiTurnAgent(
            llm_processor=llm,
            prompt_processor=PromptBuilder('prompt_templates', 'search'),
            tools=self._initialize_search_tools(project, vector_store, doc_store),
            messages=project.search_messages
        )
        return agent
        
    def _initialize_consolidate_agent_for_project(self, project: Project, llm: LLMProcessor, vector_store: VectorStore, doc_store: DocumentStore) -> MultiTurnAgent:
        """InitialiseCPerdeert en configureert de consolidate agent voor een specifiek project."""
        agent = MultiTurnAgent(
            llm_processor=llm,
            prompt_processor=PromptBuilder('prompt_templates', 'consolidate'),
            tools=self._initialize_consolidate_tools(project, vector_store, doc_store),
            messages=project.consolidate_messages
        )
        return agent
        
    def _initialize_rewrite_agent_for_project(self, project: Project, llm: LLMProcessor, vector_store: VectorStore, doc_store: DocumentStore) -> MultiTurnAgent:
        """Initialiseert en configureert de rewrite agent voor een specifiek project."""
        agent = MultiTurnAgent(
            llm_processor=llm,
            prompt_processor=PromptBuilder('prompt_templates', 'rewrite'),
            tools=self._initialize_rewrite_tools(project, vector_store, doc_store),
            messages=project.rewrite_messages
        )
        return agent

    
    def get_heavy_components(self):
        """
        Public getter to access the heavy components from anywhere.
        """
        if not self._initialized:
            raise RuntimeError("GlobalAppStore not initialized.")
        return self.llm, self.doc_store, self.vector_store

    def get_agent(self, project: Project, agent_type: AgentType) -> MultiTurnAgent:
        """
        A generic factory function to get or create an agent.
        
        It checks a global cache for an existing agent for this project.
        If not found, it uses the heavy components and the correct
        initializer to build a new agent, which is then cached and returned.
        """
        agent_key = f"{project.id}_{agent_type}"

        cached_agent = self.agents.get(agent_key)
        if cached_agent:
            return cached_agent
        
        print(f"--- Agent Cache MISS: Initializing '{agent_key}' ---")
        
        if not self.llm or not self.doc_store or not self.vector_store:
             raise RuntimeError("Heavy components not loaded.")
            
        initializer_func = self.AGENT_INITIALIZERS[agent_type]
        
        new_agent = initializer_func(project, self.llm, self.vector_store, self.doc_store)
        
        self.agents[agent_key] = new_agent
        return new_agent


global_store = GlobalAppStore()