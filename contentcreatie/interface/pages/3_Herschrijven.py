...
if getattr(active_project, 'archived', False):
    st.warning("Dit project is momenteel gearchiveerd.")
...
if not active_project.consolidated_json:
    ...
    with tab1:
        ...
        if active_project.consolidated_json:
            ...
            st.json(active_project.consolidated_json)
        else:
...
with st.sidebar:
    st.divider()
    st.subheader("Project Beheer")
    

    current_archive_status = getattr(active_project, 'archived', False)
    
    archive_toggle = st.checkbox(
        "Project Archiveren", 
        value=current_archive_status,
        help="Markeer dit project als gearchiveerd."
    )

    if archive_toggle != current_archive_status:
        active_project.archived = archive_toggle
